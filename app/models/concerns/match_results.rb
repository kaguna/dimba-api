...
module MatchResults
  ...
  def self.get_match_results(match_id)
    ...
    home_team_goals ||= match.played? ? match.result.home_goals :
     count_goals(match_commentaries: commentaries(match_id), team: match.home_team)
    away_team_goals ||= match.played? ? match.result.away_goals :
     count_goals(match_commentaries: commentaries(match_id), team: match.away_team)
    {
      fixture_id: match.id,
      match_day: match.match_day,
      home_team: team_goals(team: match.home_team, other_team: match.away_team.name,
                            match_day: match.match_day,
                            goals_for: home_team_goals,
                            goals_against: away_team_goals),
      away_team: team_goals(team: match.away_team, other_team: match.home_team.name,
                            match_day: match.match_day,
  ...
  def self.team_goals(team:, other_team:, match_day:, goals_for:, goals_against:)
    {
      ...
      points: calculate_match_points(goals_for: goals_for, goals_against: goals_against)
    }
  end
